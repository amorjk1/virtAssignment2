...
serverList = [ "amorjk1-app", "amorjk1-db", "amorjk1-web" ]

IMAGE = 'ubuntu-minimal-16.04-x86_64'
FLAVOUR = 'c1.c1r1'
NETWORK = 'amorjk1-net'
SUBNET = 'amorjk1-subnet'
ROUTER = 'amorjk1-rtr'
...
def create():
    ...
    if conn.network.find_router('amorjk1-rtr') is None:
    ...
    for serverName in serverList:
        if conn.compute.find_server(name_or_id=serverName) is None:
            ...
            if serverName == "amorjk1-web":   
                web = conn.compute.find_server("amorjk1-web")
                conn.compute.wait_for_server(server)
                conn.compute.add_floating_ip_to_server(web, floating_ip.floating_ip_address)
                print("Floating IP " + str(floating_ip.floating_ip_address) + " applied to amorjk1-web.")
...
def destroy():
    ...
    # DESTROY NETWORK
    network = conn.network.find_network("amorjk1-net")
