...
SERVERLIST = { 'amorjk1-web', 'amorjk1-app', 'amorjk1-db' }
...
def create():
    ...
    subnet = conn.network.find_subnet('amorjk1-subnet')
    router = conn.network.find_router('amorjk1-rtr')
    ...
    for server in SERVERLIST:
        ...
        if check_server:
			...
			#If a server exists with the name "amorjk1-web", give it floating ip
            if server == 'amorjk1-web':
                floating_ip = conn.network.create_ip(floating_network_id=public_net.id)
                web_server = conn.compute.find_server('amorjk1-web')
...
def destroy():
    ...
    for server in SERVERLIST:
        ...
        #Remove floating ip off the "amorjk1-web" server (if it exists)
        if check_server:
            if server == 'amorjk1-web':
...
def status():
    ...
    for server in SERVERLIST:
        ...
        if check_server:
            ...
            #Display floating ip for "amorjk1-web" server
            if server == 'amorjk1-web':
